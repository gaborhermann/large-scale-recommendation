...
class FlinkOnlineMF(iterationWaitTime: Long = 0) {

  private val log = LoggerFactory.getLogger(classOf[FlinkOnlineMF])

  def buildModel(ratings: DataStream[Rating],
                 factorInit: FactorInitializerDescriptor,
                 factorUpdate: FactorUpdater,
                 parameters: Map[String, String] = Map.empty):
  DataStream[(UserVector, ItemVector)] = {

    def stepFunc(users: ConnectedStreams[Rating, UserVector]):
    (DataStream[UserVector], DataStream[(UserVector, ItemVector)]) = {

      val ratingsWithUserVector: DataStream[(Rating, Array[Double])] =
